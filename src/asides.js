window.SONSHI_ASIDES = [
  {
    title: "十三篇を束ねた武将、経営に持ち込んだ起業家",
    layers: [
      {
        era: "三国時代",
        text: "いま読まれている十三篇は、魏の曹操が整理し、注を付けた本の系統を土台にしています。乱世を戦い抜いた武将自身が、始計の五事や七計を読み解き、後の読者へ手渡したのです。",
      },
      {
        era: "昭和の日本",
        text: "ソフトバンクの孫正義氏は、二十代で病に伏していた時期に、孫子とランチェスター戦略を組み合わせた「孫の二乗の兵法」をまとめたとされます。五行五列の二十五文字のうち、一行目の「道天地将法」は始計の五事を、四行目の「智信仁勇厳」は将の五つの徳を、そのまま並べたものです。",
      },
    ],
    takeaway: "二千年を隔てて引き継がれたのは、奇策ではなく、戦う前の点検表でした。",
    mentions: [
      {
        where: "北宋 武経七書の筆頭に",
        text: "神宗の時代、武官を選ぶ試験のために七つの兵法書が「武経七書」と定められ、孫子はその最初に置かれました。日本でも慶長十一年（1606年）、徳川家康の命で刊行された伏見版に収められ、江戸の兵学の土台になります。",
      },
      {
        where: "2001年 米ドラマ『ザ・ソプラノズ』",
        text: "マフィアのボス、トニー・ソプラノが、精神科医に勧められた孫子を読んで役に立っていると語る場面が放送されると、英訳本が急に売れ始めました。ある出版社では、ふだん週に数百部の本が四月末までに一万四千部売れたといいます。",
      },
    ],
    sources: [
      { label: "Wikipedia「孫子 (書物)」", url: "https://ja.wikipedia.org/wiki/%E5%AD%AB%E5%AD%90_(%E6%9B%B8%E7%89%A9)" },
      { label: "ダイヤモンド・オンライン「孫の2乗の兵法」", url: "https://diamond.jp/articles/-/257022" },
      { label: "Wikipedia「武経七書」", url: "https://ja.wikipedia.org/wiki/%E6%AD%A6%E7%B5%8C%E4%B8%83%E6%9B%B8" },
      { label: "The Baltimore Sun「Hey, if Tony's reading it, it's got to be good」", url: "https://www.baltimoresun.com/news/bs-xpm-2001-05-13-0105130365-story.html" },
    ],
  },
  {
    title: "褒め言葉が、けなし言葉になるまで",
    layers: [
      {
        era: "原典",
        text: "孫子は、拙くても速く終えた戦は聞くが、巧みな長期戦が国を益した例は見たことがない、と述べました。比べているのは、短い戦と、長引く戦です。",
      },
      {
        era: "後の世",
        text: "この一節はやがて「兵は拙速を尊ぶ」「巧遅は拙速に如かず」といった格言の形で語られ、仕事の速さを説く場面にも広がりました。",
      },
      {
        era: "いまの日本語",
        text: "ところが今、「拙速な判断」と言えば、ほとんど非難の言葉です。同じ二字が、称賛にも批判にも使われています。",
      },
    ],
    takeaway: "「拙速」の二字は、何と比べるかによって、褒め言葉にもけなし言葉にもなります。",
    mentions: [
      {
        where: "三国時代 郭嘉の献策",
        text: "曹操が遠く烏桓を討とうとしたとき、参謀の郭嘉は「兵は神速を貴ぶ」と説き、重い荷を残して軽装の兵で急がせるよう進言しました。作戦篇の「兵は勝つことを貴んで、久しきを貴ばず」と響き合う言葉です。進言を受けたのは、孫子に注を付けた曹操その人でした。",
      },
    ],
    sources: [
      { label: "Web漢文大系「兵は拙速を尊ぶ」", url: "https://kanbun.info/koji/heiwasesso.html" },
      { label: "故事ことわざ辞典「巧遅は拙速に如かず」", url: "https://kotowaza-dictionary.jp/k0858/" },
      { label: "Wikipedia「郭嘉」", url: "https://ja.wikipedia.org/wiki/%E9%83%AD%E5%98%89" },
    ],
  },
  {
    title: "革命家も、受験生も引いた一句",
    layers: [
      {
        era: "1936年の中国",
        text: "毛沢東は『中国革命戦争の戦略問題』で「知彼知己、百戦不殆」を孫武子の言葉として引き、学ぶ段階と使う段階の両方を含む言葉だから軽く見てはならない、と書きました。",
      },
      {
        era: "いまの日本",
        text: "日本では「敵を知り己を知れば」と言い換えて引かれることが多く、試合前の心構えから受験の標語、就職活動の自己分析まで、さまざまな場面で使われています。",
      },
      {
        era: "原文の続き",
        text: "ただ、原文が約束するのは「百戦百勝」ではなく「殆うからず」、つまり危うくならないことです。さらに「己だけを知れば一勝一負」と続きます。",
      },
    ],
    takeaway: "原文が約束しているのは必勝ではなく、危うくならないことでした。",
    mentions: [
      {
        where: "1987年 米ドラマ『新スタートレック』",
        text: "第一シーズンの一話「The Last Outpost」で、副長ライカーが孫子を引きます。戦うべき時と戦うべきでない時を知る者が勝つ。謀攻篇で「勝を知るに五あり」の最初に挙がる一句が、二十四世紀の宇宙船の上でも語られていました。",
      },
    ],
    sources: [
      { label: "毛沢東「中国革命戦争の戦略問題」（中国語）", url: "https://www.marxists.org/chinese/maozedong/marxist.org-chinese-mao-193612.htm" },
      { label: "Web漢文大系「彼を知り己を知れば百戦殆からず」", url: "https://kanbun.info/koji/karewoshiri.html" },
      { label: "IMDb「The Last Outpost」Quotes", url: "https://www.imdb.com/title/tt0708804/quotes/" },
    ],
  },
  {
    title: "二千年前の竹簡には、逆に書かれていた",
    layers: [
      {
        era: "通行本",
        text: "私たちが読む本文は「守れば則ち足らず、攻むれば則ち余り有り」。力が足りないから守り、余っているから攻める、という読みです。",
      },
      {
        era: "1972年 銀雀山",
        text: "山東省の前漢の墓から見つかった竹簡の孫子では、この箇所が「守れば則ち余り有り、攻むれば則ち足らず」と逆の形で記されていました。どちらが本来の形かは、研究者の間でも読みが分かれています。同じ墓からは『孫臏兵法』も見つかり、孫武と孫臏の兵法が別の書であったことも確かめられました。",
      },
      {
        era: "現代の米海兵隊",
        text: "米海兵隊の教範『MCDP 1 Warfighting』は、通行本の読みに沿って、この一節を英訳で引いています。",
      },
    ],
    takeaway: "どちらの読みを取っても、攻めるか守るかを力の多寡で決めるという点は変わりません。",
    mentions: [
      {
        where: "1987年 映画『ウォール街』",
        text: "投資家ゴードン・ゲッコーは若い部下に、自分は確実な勝負にしか賭けないと言い、孫子を読め、どの戦いも戦う前に勝負はついている、と続けます。軍形篇の「勝兵は先づ勝つて後ちに戰を求む」を、金融の世界へ持ち込んだ台詞です。",
      },
    ],
    sources: [
      { label: "熊征「『孫子』形篇の攻守観について」（北海道大学）", url: "https://eprints.lib.hokudai.ac.jp/dspace/bitstream/2115/79796/1/09_rjgshhs_19_p157-188_l.pdf" },
      { label: "Wikipedia「銀雀山漢簡」", url: "https://ja.wikipedia.org/wiki/%E9%8A%80%E9%9B%80%E5%B1%B1%E6%BC%A2%E7%B0%A1" },
      { label: "U.S. Marine Corps「MCDP 1」", url: "https://www.marines.mil/News/Publications/MCPEL/Electronic-Library-Display/Article/899837/mcdp-1/" },
      { label: "IMDb『ウォール街』Quotes", url: "https://www.imdb.com/title/tt0094291/quotes/" },
    ],
  },
  {
    title: "西へ渡った十三篇",
    layers: [
      {
        era: "1772年 パリ",
        text: "中国に滞在したイエズス会の宣教師アミオが仏訳を刊行し、孫子は初めてヨーロッパの言葉で読まれるようになりました。",
      },
      {
        era: "1963年 英語圏",
        text: "米海兵隊の将官だったグリフィスの英訳に、英国の戦略思想家リデル・ハートが序文を寄せ、戦争研究への最良の短い入門書だと評しました。リデル・ハートは、敵の予期しない方向から崩す「間接アプローチ」を唱えた人物です。",
      },
      {
        era: "読み合わせ",
        text: "正面で受け止め、思わぬ所から決める。兵勢篇の「正」と「奇」は、この間接アプローチと並べて読まれることがあります。",
      },
    ],
    takeaway: "大陸と二千年を隔て、別の名で語り直されながら、同じ考えが読み継がれてきました。",
    mentions: [
      {
        where: "唐『李衛公問対』",
        text: "唐の太宗と名将・李靖の問答をまとめた書で、その多くが孫子の奇正や虚実の読み解きにあてられています。見せ方しだいで奇は正に、正は奇に変わる、と踏み込んだこの書も、のちに孫子と並んで武経七書に数えられました。",
      },
      {
        where: "1905年 東京",
        text: "孫子を初めて英語に訳したのは、日本に駐在していた英国の砲兵将校カルスロップでした。書名は日本語読みの「SONSHI」。二人の日本人の助けを借りた訳で、のちに中国学者ジャイルズから厳しく批判され、1910年にはジャイルズ自身の英訳が出ています。",
      },
    ],
    sources: [
      { label: "Wikipedia「The Art of War」（英語）", url: "https://en.wikipedia.org/wiki/The_Art_of_War" },
      { label: "Wikisource『李衛公問対』", url: "https://ja.wikisource.org/wiki/%E6%9D%8E%E8%A1%9B%E5%85%AC%E5%95%8F%E5%AF%BE" },
      { label: "Project Gutenberg『Sun Tzŭ on The Art of War』（ジャイルズ訳の序文）", url: "https://www.gutenberg.org/files/132/132-h/132-h.htm" },
    ],
  },
  {
    title: "米海兵隊の教範に流れる水",
    layers: [
      {
        era: "現代の米海兵隊",
        text: "教範『MCDP 1 Warfighting』は、敵の強い所を「サーフェス（面）」、弱い所を「ギャップ（隙間）」と呼び、面を避けて隙間へ入り込めと説きます。その強弱は兵力だけでなく、心や士気にもあると付け加えています。",
      },
      {
        era: "原典",
        text: "水は高きを避けて低きに流れ、兵は実を避けて虚を撃つ。虚実篇の水の喩えは、この考え方を二千年以上前に言い表していました。",
      },
      {
        era: "読み比べ",
        text: "水を「器に従うもの」と見る言い回しもあります。けれど孫子の水は受け身ではありません。低い所を探して、自ら流れ込む水です。",
      },
    ],
    takeaway: "同じ水の喩えも、柔らかさと読むか、隙へ流れ込む鋭さと読むかで、表情を変えます。",
    mentions: [
      {
        where: "江戸時代の浄瑠璃",
        text: "「虚々実々の駆け引き」と言うときの「虚々実々」は、虚実篇の虚と実を重ねた言葉です。『日本国語大辞典』が早い用例に挙げるのは、近松門左衛門の浄瑠璃『信州川中島合戦』（1721年）。謙信と信玄が秘術を尽くして斬り結ぶ場面で、信玄は孫子の心を練る、と語られます。",
      },
    ],
    sources: [
      { label: "U.S. Marine Corps「MCDP 1」", url: "https://www.marines.mil/News/Publications/MCPEL/Electronic-Library-Display/Article/899837/mcdp-1/" },
      { label: "Marine Corps University「Warfighting Cliff Notes」", url: "https://www.usmcu.edu/Portals/218/mcdp1%20warfighting%20lab.pdf" },
      { label: "コトバンク「虚虚実実」", url: "https://kotobank.jp/word/%E8%99%9A%E8%99%9A%E5%AE%9F%E5%AE%9F-479281" },
    ],
  },
  {
    title: "旗から小説へ、そして二十五文字へ",
    layers: [
      {
        era: "戦国時代",
        text: "武田信玄は軍争篇の一節を記した旗を掲げたと伝わり、そこから四字の「風林火山」が武田の代名詞になりました。",
      },
      {
        era: "昭和から平成",
        text: "井上靖の小説『風林火山』が信玄の軍師・山本勘助を描き、2007年にはNHK大河ドラマの題にもなりました。",
      },
      {
        era: "現代の経営",
        text: "ソフトバンクの孫正義氏が掲げる「孫の二乗の兵法」では、最後の行が「風林火山海」。原典の四字に海を足して、五字にしています。",
      },
    ],
    takeaway: "原文は風・林・火・山のあと、陰と雷へ続きます。旗に記されなかった二句では、隠れる時と動く時が語られています。",
    mentions: [
      {
        where: "山梨県甲州市 雲峰寺",
        text: "信玄の軍旗と伝わる「孫子の旗」が、今もこの寺に残っています。金泥で大書された「疾如風 徐如林 侵掠如火 不動如山」の十四字。恵林寺の快川紹喜に書かせたと伝えられ、日本最古とされる日の丸の御旗とともに宝物殿に並んでいます。",
      },
    ],
    sources: [
      { label: "Wikipedia「風林火山」", url: "https://ja.wikipedia.org/wiki/%E9%A2%A8%E6%9E%97%E7%81%AB%E5%B1%B1" },
      { label: "Wikipedia「風林火山 (NHK大河ドラマ)」", url: "https://ja.wikipedia.org/wiki/%E9%A2%A8%E6%9E%97%E7%81%AB%E5%B1%B1_(NHK%E5%A4%A7%E6%B2%B3%E3%83%89%E3%83%A9%E3%83%9E)" },
      { label: "ダイヤモンド・オンライン「孫の2乗の兵法」", url: "https://diamond.jp/articles/-/257022" },
      { label: "山梨市「孫子の旗」", url: "https://www.city.yamanashi.yamanashi.jp/site/cultural-assets/1658.html" },
      { label: "富士の国やまなし観光ネット「風林火山史跡巡り」", url: "https://www.yamanashi-kankou.jp/history/shingen/forest.html" },
    ],
  },
  {
    title: "王の寵姫を斬った将軍",
    layers: [
      {
        era: "春秋時代の呉",
        text: "『史記』によれば、孫武は呉王闔閭に試され、宮中の女性百八十人を兵として調練しました。号令に笑って従わない隊長役の寵姫二人を斬ろうとし、王が止めても「将、軍に在りては、君命も受けざる所あり」と退けたといいます。",
      },
      {
        era: "伝記として",
        text: "『史記』が孫武について詳しく語る場面は、ほとんどこの一話です。そのため、この逸話が孫武という人物像そのものになりました。",
      },
      {
        era: "読み方",
        text: "苛烈な話です。命令が守られる軍は平時の規律から生まれる、という話として読まれることもあれば、将が君主の意に背く危うさを示す話として読まれることもあります。九変篇の「君命も受けざる所あり」も、同じように幅をもって読まれてきました。",
      },
    ],
    takeaway: "孫武の逸話も周亜夫の逸話も、陣中では将の令が君命より重く扱われた場面として語り継がれてきました。",
    mentions: [
      {
        where: "前漢 細柳の陣",
        text: "匈奴に備えて細柳に陣を敷いた周亜夫のもとへ、文帝が自ら慰問に訪れました。門の兵は「軍中では将軍の令を聞き、天子の詔は聞かない」と答えて通さず、文帝は使者を立ててから、ようやく馬をゆっくり進めて入りました。帰り道、文帝は、これこそ真の将軍だと称えたといいます。",
      },
    ],
    sources: [
      { label: "Wikisource『史記』孫子呉起列伝（中国語）", url: "https://zh.wikisource.org/wiki/%E5%8F%B2%E8%A8%98/%E5%8D%B7065" },
      { label: "Wikisource『史記』絳侯周勃世家（中国語）", url: "https://zh.wikisource.org/wiki/%E5%8F%B2%E8%A8%98/%E5%8D%B7057" },
    ],
  },
  {
    title: "雁の列が乱れたとき",
    layers: [
      {
        era: "平安時代 後三年の役",
        text: "源義家は金沢柵へ向かう途中、空を渡る雁の列が急に乱れるのを見て、野に伏兵がいると見抜き、潜んでいた敵を討ち取ったと伝わります。",
      },
      {
        era: "学びの由来",
        text: "義家は以前、学者の大江匡房から「優れた武者だが兵法を知らない」と評され、それをきっかけに匡房に兵法を学んだとされます。雁の乱れは伏兵のしるし、という教えはそのとき授かったものでした。行軍篇の「鳥起つは伏なり」と重なる教えです。",
      },
    ],
    takeaway: "伝承では、義家が雁の乱れを読めたのは、学んだ兵法があったからだとされています。",
    sources: [
      { label: "Wikipedia「後三年の役」", url: "https://ja.wikipedia.org/wiki/%E5%BE%8C%E4%B8%89%E5%B9%B4%E3%81%AE%E5%BD%B9" },
    ],
  },
  {
    title: "将軍が、兵の傷を吸った話",
    layers: [
      {
        era: "戦国時代",
        text: "『史記』で孫武と同じ列伝に並ぶ将軍・呉起は、兵と同じものを食べ、同じ所に寝て、腫れ物を患った兵の膿を自ら口で吸い出したといいます。",
      },
      {
        era: "母の涙",
        text: "それを聞いた兵の母は泣きました。夫も同じように膿を吸われ、恩に報いようと退かずに戦って死んだ。今度は息子の番だ、と。",
      },
      {
        era: "地形篇と並べると",
        text: "地形篇は「卒を視ること愛子の如し、故に之と倶に死すべし」と説き、同時に、甘やかすだけなら兵は「驕子」になると戒めます。慈しみは、人を死地へ向かわせるほどの力を持つのです。",
      },
    ],
    takeaway: "将の情けを、母は恩としてではなく、息子を失う前触れとして受け取りました。",
    sources: [
      { label: "Wikisource『史記』孫子呉起列伝（中国語）", url: "https://zh.wikisource.org/wiki/%E5%8F%B2%E8%A8%98/%E5%8D%B7065" },
      { label: "Wikipedia「呉起」", url: "https://ja.wikipedia.org/wiki/%E5%91%89%E8%B5%B7" },
    ],
  },
  {
    title: "背水の陣と、奈良時代の孫子講義",
    layers: [
      {
        era: "楚漢戦争",
        text: "漢の将・韓信は川を背に陣を敷きました。将たちに理由を問われると「兵法に、死地に陥れて後に生く、と言うではないか」と答えます。九地篇の一節です。ここから「背水の陣」という言葉が生まれました。",
      },
      {
        era: "奈良時代の日本",
        text: "『続日本紀』には、天平宝字四年（760年）、大宰府にいた吉備真備が、都から送られた武官に「諸葛亮の八陣」と「孫子の九地」を教えたと記されています。日本で孫子が学ばれた早い記録で、選ばれていたのがこの九地篇でした。",
      },
      {
        era: "言葉の裏側",
        text: "「背水の陣」は今も、後がない状況で覚悟を決める意味で使われます。ただし韓信は、事前に別働隊を敵の砦の背後へ回り込ませていました。",
      },
    ],
    takeaway: "覚悟だけで勝ったのではありません。退路を断つ前に、勝ち筋を別に用意していたのです。",
    sources: [
      { label: "Web漢文大系「背水の陣」", url: "https://kanbun.info/koji/haisui.html" },
      { label: "Wikipedia「吉備真備」", url: "https://ja.wikipedia.org/wiki/%E5%90%89%E5%82%99%E7%9C%9F%E5%82%99" },
    ],
  },
  {
    title: "孫子に注を付けた人が、火に敗れた",
    layers: [
      {
        era: "208年 赤壁",
        text: "孫子に注を付けた曹操その人が、赤壁で呉の黄蓋の火攻めを受け、大敗しました。強い風に乗った火が船団を焼いたと伝わります。",
      },
      {
        era: "222年 夷陵",
        text: "関羽を討たれた後、蜀の劉備は呉へ大軍を進めましたが、陸遜に陣営を火で焼かれて敗れます。この戦は、怒りから起こした戦としても語られてきました。",
      },
      {
        era: "火攻篇と並べると",
        text: "火攻篇は火の使い方を説いた後、「主は怒りを以て師を興すべからず」と結びます。二つの戦は、火の条件を読んだ側と読まれた側の物語であり、夷陵はその結びの戒めとも重なります。",
      },
    ],
    takeaway: "兵法を誰より深く読んだ者でも、火に敗れることがありました。",
    sources: [
      { label: "Wikipedia「赤壁の戦い」", url: "https://ja.wikipedia.org/wiki/%E8%B5%A4%E5%A3%81%E3%81%AE%E6%88%A6%E3%81%84" },
      { label: "Wikipedia「夷陵の戦い」", url: "https://ja.wikipedia.org/wiki/%E5%A4%B7%E9%99%B5%E3%81%AE%E6%88%A6%E3%81%84" },
    ],
  },
  {
    title: "噂ひとつで、名将を退けた",
    layers: [
      {
        era: "戦国時代の斉",
        text: "燕の名将・楽毅に国土の大半を奪われた斉で、即墨を守る田単は燕へ噂を流しました。楽毅は斉で王になるつもりだ、と。即位したばかりの燕の恵王は楽毅を退けて将を代え、田単はその後、燕軍を破って失った城を取り戻しました。",
      },
      {
        era: "名前になった計",
        text: "敵の内に疑いを生む手は、のちに兵法書『三十六計』の一つ「反間計」として名を残します。",
      },
      {
        era: "用間篇と並べると",
        text: "恵王は太子の頃から楽毅と折り合いが悪かったとされます。噂が効いたのは、燕の君臣の間にあった疑いの芽を、田単が知っていたからでした。孫子が敵の内情を知る人を重んじた理由が、ここにあります。",
      },
    ],
    takeaway: "一つの噂が、城を攻めるより先に、敵の名将を戦場から退けました。",
    mentions: [
      {
        where: "江戸中期 荻生徂徠『孫子国字解』",
        text: "儒学者の荻生徂徠は、孫子を漢文ではなく仮名交じりの日本語で読み解く注釈を書きました。初学者向けの体裁ながら、優れた孫子注として今も読まれています。",
      },
      {
        where: "幕末の長州 松下村塾",
        text: "長州藩の兵学師範だった吉田松陰は、最晩年に松下村塾で孫子を講じ、『孫子評註』を遺しました。江戸の孫子研究を集大成しつつ、欧米列強が迫る時勢に引き寄せて読んだもので、高杉晋作や久坂玄瑞ら門下生の戦略の源になったとも言われます。",
      },
    ],
    sources: [
      { label: "Wikisource『史記』田単列伝（中国語）", url: "https://zh.wikisource.org/wiki/%E5%8F%B2%E8%A8%98/%E5%8D%B7082" },
      { label: "Wikipedia「三十六計」", url: "https://ja.wikipedia.org/wiki/%E4%B8%89%E5%8D%81%E5%85%AD%E8%A8%88" },
      { label: "河出書房新社『荻生徂徠全集8 孫子國字解 他』", url: "https://www.kawade.co.jp/np/isbn/9784309605586/" },
      { label: "PHP研究所『吉田松陰『孫子評註』を読む』", url: "https://www.php.co.jp/books/detail.php?isbn=978-4-569-84192-2" },
    ],
  },
];
